...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 16 20:34:47 2025
...
# -*- coding: utf-8 -*-
"""
Async Robust AFL (baseline) on MNIST
...
- Saves: per-client losses, server loss proxy, selected clients, exec times,
         test loss/acc, and staleness stats for easy comparison to RAFL.
...
# ----------------- Utils -----------------
def create_directory(tag: str, base_dir="results_afl_baseline_mnist"):
...
def dict_to_vec(update: Dict[str, torch.Tensor]) -> torch.Tensor:
    parts = [v.detach().flatten().float() for v in update.values() if v.dtype.is_floating_point]
    if len(parts) == 0:
        return torch.zeros(1)
    return torch.cat(parts)
...
async def federated_learning_afl_baseline(
    ...
    for t in tqdm(range(num_rounds), desc="AFL Baseline Rounds (MNIST)"):
...
# ----------------- Main -----------------
async def main():
    # ---- Hyperparams (aligned with your MNIST RAFL defaults) ----
    num_clients = 10
    alpha_dirichlet = 0.5
    num_rounds = 500
    local_epochs = 3
    ...
    tag = (f"mnist_clients_{num_clients}_rounds_{num_rounds}_epochs_{local_epochs}_"
           f"cpr_{num_clients_per_round}_agg_{aggregator}")
    results_dir = create_directory(tag, base_dir="results_afl_baseline_mnist")
    ...
    print(f"[DONE] Baseline AFL (MNIST) results saved in: {results_dir}")
