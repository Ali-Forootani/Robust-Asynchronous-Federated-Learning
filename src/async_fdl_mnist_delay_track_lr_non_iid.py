...
def create_directory(num_clients, num_rounds, local_epochs, max_clients_per_round, base_dir="results"):
    dir_name = f"{base_dir}/mnist_clients_{num_clients}_rounds_{num_rounds}_epochs_{local_epochs}_clients_per_round_{max_clients_per_round}_{time.strftime('%Y%m%d_%H%M%S')}"
...
def partition_non_iid(dataset, num_clients, alpha=0.5):
    """
    Partition the dataset non-IID using Dirichlet distribution.
    """
