...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 15 09:36:12 2025
...
# -*- coding: utf-8 -*-
"""
Robust Asynchronous Federated Learning (RAFL) on MNIST
with round-wise test evaluation and heterogeneity proxy \hat{\nu}_t^2.

This is a faithful port of your CIFAR-10 RAFL script to MNIST.
Key changes vs CIFAR version:
- Dataset: torchvision.datasets.MNIST (1x28x28) + appropriate normalization.
- Model: same small ResNet-style backbone but with 1 input channel.
- Results directory: results_rafl_mnist/...
- Modest defaults for local epochs / batch size for MNIST (tune as you like).

HPC/Headless safety retained:
- DataLoader num_workers=0 (critical when iterated from background threads).
- Multiprocessing start method 'spawn'.
- Matplotlib backend 'Agg'.

Run:
    python rafl_mnist.py
"""
...
# ---- HPC stability: set safe start method BEFORE any torch DataLoader workers
...
# ------------------------- Utilities -------------------------
def create_directory(num_clients, num_rounds, local_epochs, max_clients_per_round, base_dir="results_rafl_mnist"):
    dn = os.path.join(
        base_dir,
        f"mnist_clients_{num_clients}_rounds_{num_rounds}_epochs_{local_epochs}_"
...
def build_client_loaders(
    ...
    """
    Split each client's indices into train and a small validation slice (for \hat{\nu}_t^2).
...
async def federated_learning_rafl(
    ...
    num_rounds=50, local_epochs=3, max_clients_per_round=5,
    ...
    if alpha_t >= 0.5:
        print(f"[WARN] η^2 L^2 I(I-1) = {alpha_t:.3f} ≥ 1/2. Consider smaller η and/or I.")
    ...
    for t in tqdm(range(num_rounds), desc="RAFL Rounds (MNIST)"):
        ...
        s_min_series.append(w_min * (1.0 - float(B_round) / max(1, J_per_round)))
    ...
    metrics = {
        ...
        "gamma_t": gamma_ts,
        "kappa_sq": max((w_max - 1.0) ** 2, (1.0 - w_min) ** 2),
        "rho_sb_hat": [],  # optional: return from asb_aggregate if you want; currently unused for MNIST plots
        "s_min": s_min_series,
        "D_T": DT,
        "alpha_t": (eta_client ** 2) * (L_smooth ** 2) * I * (I - 1),
        "nu2": nu2_series,
...
async def main():
    ...
    # ---- AFL mode (turn RAFL -> vanilla AFL) ----
    AFL_MODE = True
    if AFL_MODE:
        enable_byzantine = False   # no Byzantine sim
        byz_frac = 0.0             # irrelevant when disabled
        asb_trim = False           # no trimming
        trigger_eps = 0.0          # include all updates (S = all selected)
        alpha_stale = 0.0          # no staleness weighting
        beta_et = 0.0              # no event-trigger scaling
        w_min, w_max = 1.0, 1.0    # force uniform 1/J weights
    ...
    def make_model():
        return ResNetMNIST(BasicBlock, [2, 2, 2], num_classes=10)
    ...
    print(f"[DONE] MNIST results saved in: {results_dir}")
