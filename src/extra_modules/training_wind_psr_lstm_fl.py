...
hidden_features = 32
hidden_layers = 4  # Number of LSTM layers
...
for i, (client_coords, ith_client_data) in enumerate(client_data):
    ...
    test_loader) = client_instance.prepare_data_random(test_data_size=0.999)
...
"""
# Run the federated learning process and capture training losses
global_model, training_losses = federated_learning_2(
...
# Evaluate each client on the test dataset
...
server_model, training_losses = federated_learning_9(
    clients_models = clients_models,
    server_model= global_model,
    clients_train_loaders= clients_train_loaders,
    clients_test_loaders= clients_test_loaders,
...
"""
# After federated learning is done, evaluate each client on the test dataset and save losses
for i, client_model in enumerate(clients_models):
    client_model.eval()
    total_loss = 0
    total_samples = 0
...
# Track losses for each client during training and save them as NumPy arrays
for i, losses in enumerate(training_losses):
    # Convert the list of losses to a NumPy array
    np_losses = np.array(losses)
    
    # Save each client's loss history as a separate NumPy file
    np.save(f'client_{i + 1}_training_losses.npy', np_losses)
