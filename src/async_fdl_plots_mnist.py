...
def load_and_plot_saved_losses(results_dir):
    ...
    if os.path.exists(server_losses_file):
        ...
        #plt.yscale("log")
        plt.xscale("log")
        plt.title("Server Loss Across Rounds in MNIST dataset with 50% clients participation", fontsize=18)
        plt.legend()
        ...
        plt.savefig(root_dir + "/results/" +"asynch_nonconvex_mnist_server_losses_clients_5.png", dpi=300, bbox_inches='tight')  # High-quality PNG
        plt.savefig(root_dir + "/results/"+ "asynch_nonconvex_mnist_server_losses_clients_5.pdf", bbox_inches='tight')  # PDF format
        plt.show()
    else:
...
# Example usage
results_dir = root_dir + "/results/mnist_clients_10_rounds_1000_epochs_10_clients_per_round_7_20250219_120159"  # Replace with your actual results directory
...
# Load the CSV file
file_path = root_dir + "/results/mnist_clients_10_rounds_1000_epochs_10_clients_per_round_7_20250219_120159"+"/selected_clients.csv"  # Update if needed
...
plt.title("Selected Clients for MNIST with 50% client participation", fontsize=18)
...
plt.savefig(root_dir + "/results/" +"histogram_afl_mnist_clients_5.png", dpi=300, bbox_inches='tight')  # High-quality PNG
plt.savefig(root_dir + "/results/"+ "histogram_afl_mnist_clients_5.pdf", bbox_inches='tight')  # PDF format
plt.show()
...
plt.xlabel("Client IDs", fontsize=18)
plt.ylabel("Frequency", fontsize=18)
plt.title("Selected Clients for MNIST with 50% client participation", fontsize=16)
