...
# Find all directories that start with "mnist"
mnist_dirs = [d for d in os.listdir(root_dir) if d.startswith("mnist") and os.path.isdir(os.path.join(root_dir , d))]

# Initialize a dictionary to store server losses
server_losses_dict = {}

# Load server losses from each directory
for mnist_dir in mnist_dirs:
    server_losses_path = os.path.join(root_dir, mnist_dir, "server_losses.npy")
    
    if os.path.exists(server_losses_path):
        server_losses = np.load(server_losses_path)
        server_losses_dict[mnist_dir] = server_losses

# Plot server losses for each directory
plt.figure(figsize=(10, 6))
for mnist_dir, server_losses in server_losses_dict.items():
    plt.plot(server_losses, label=mnist_dir)

plt.xlabel("Rounds")
plt.ylabel("Loss")
plt.title("Server Loss Across Rounds for Different MNIST Experiments")
plt.xscale("log")
plt.legend()
plt.grid(True)
plt.show()
    ...
    if os.path.exists(server_losses_path):
        ...
        if isinstance(server_losses, np.ndarray):
            try:
                ...
                print(f"Skipping {mnist_dir}: Unable to convert to uniform NumPy array.")
        else:
            print(f"Skipping {mnist_dir}: Inconsistent type {type(server_losses)}")

# Debugging: Print directory names and extracted parts
for mnist_dir in server_losses_dict.keys():
    parts = mnist_dir.split("_")
    print(f"Directory: {mnist_dir}, Split Parts: {parts}")
...
for idx, (mnist_dir, server_losses) in enumerate(server_losses_dict.items()):
    parts = mnist_dir.split("_")
    ...
    try:
        ...
        print(f"Skipping {mnist_dir}: Unexpected format")
        continue
    ...
    if server_losses.ndim == 1:
        ...
        print(f"Skipping {mnist_dir}: Unexpected shape {server_losses.shape}")
...
plt.title("Server Loss in AFL for Different MNIST Experiments", fontsize=18)
...
plt.savefig(root_dir + "/pictures" + "/nonconvex_mnist_server_losses_clients_participation.png", dpi=300, bbox_inches='tight')  # High-quality PNG
plt.savefig(root_dir + "/pictures" + "/nonconvex_mnist_server_losses_client_participation.pdf", bbox_inches='tight')  # PDF format
