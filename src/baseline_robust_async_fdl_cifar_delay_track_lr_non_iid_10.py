...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 16 17:06:14 2025
...
# -*- coding: utf-8 -*-
"""
Async Robust AFL (baseline) on CIFAR-10
...
- Saves: per-client losses, server loss proxy, selected clients, exec times,
         test loss/acc, and basic staleness stats (for comparability)

This is a *baseline* to compare against your RAFL code:
- Fixed server step-size gamma (no composite ζ_t / γ_t schedule)
- No event-trigger suppression p_t, no ASB trimming tied to staleness
- Simple staleness weighting (only for 'stale_mean' aggregator)
- Same small ResNet as in your RAFL code, CIFAR-10 transforms, HPC safety
"""
...
# ----------------- Utils -----------------
def create_directory(tag: str, base_dir="results_afl_baseline"):
...
def build_client_loaders(
    ...
    val_size_per_client: int = 256, num_workers: int = 0, pin_memory: bool = False
):
...
def _keys_float(updates: List[Dict[str, torch.Tensor]]):
    # assure keys from first update, float only
...
def agg_trimmed_mean(updates: List[Dict[str, torch.Tensor]], trim_ratio: float = 0.2):
    ...
    agg = {}
    for k in keys:
        stacked = torch.stack([u[k].float().cpu() for u in updates], dim=0)  # [J, ...]
...
def agg_krum(updates: List[Dict[str, torch.Tensor]], byz_frac: float = 0.3, m_multi: int = 1):
    ...
    # Krum score: sum of smallest J - f - 2 distances
    scores = []
    keep_k = max(1, J - f - 2)
    for i in range(J):
        row = dmat[i].clone()
    ...
    return agg_mean(chosen_updates)  # average the m selected
...
async def federated_learning_afl_baseline(
    ...
    clients_val_loaders: List[DataLoader],  # kept for nu2 batch if you want it later
    test_loader: DataLoader,
    num_rounds=200, local_epochs=5, max_clients_per_round=5,
    max_parallel_clients=None,
    loss_fn=None,
    eta_client=5e-3,
    gamma_server=1e-2,           # fixed server step-size (contrast to your γ_t)
    ...
    # keep short history to simulate staleness
    ...
    tau_bar = []  # just for monitoring staleness
    test_loss_series = []
    test_acc_series = []
    ...
    for t in tqdm(range(num_rounds), desc="AFL Baseline Rounds"):
        ...
        # launch clients with their (possibly stale) initial states
        tasks = []
        for i in selected:
            tau_i = tau_c_map[i]
            hist_idx = -1 - tau_i
        ...
        # Byzantine simulation (applied *before* aggregation)
        ...
        # Monitor weighted avg staleness (using the same weighting as stale_mean if used)
        if len(taus_J) > 0:
...
# ----------------- Main -----------------
async def main():
    # ---- Hyperparams (feel free to mirror your RAFL settings) ----
    num_clients = 10
    alpha_dirichlet = 0.5
    num_rounds = 200
    local_epochs = 10
    ...
    # staleness weighting / robust params
    alpha_stale = 0.2
    w_min, w_max = 0.5, 1.5
    trim_ratio = 0.2
    krum_m = 1
    ...
    tag = (f"cifar_clients_{num_clients}_rounds_{num_rounds}_epochs_{local_epochs}_"
           f"cpr_{num_clients_per_round}_agg_{aggregator}")
    results_dir = create_directory(tag, base_dir="results_afl_baseline")
    ...
    print(f"[DONE] Baseline AFL results saved in: {results_dir}")
