...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 14 09:36:49 2025
...
def unflatten_to_state_dict(vec, keys, shapes, ref_state):
    ...
    for k in keys:
        ...
        out[k] = vec[offset:offset+n].view(shapes[k]).to(dtype=ref_state[k].dtype, device=ref_state[k].device)
        offset += n
    return out
...
async def client_local_update(
    ...
    # IMPORTANT: plain SGD as in the analysis (momentum helps but keeps SGD form)
...
def evaluate(model, data_loader, device):
    ...
    for x, y in data_loader:
        ...
        pred = logits.argmax(1); tot_correct += int((pred == y).sum().item()); tot += x.size(0)
...
async def run_rafl(
    ...
    rounds=20, clients_per_round=6, I_local=1, eta_client=6e-3,
    tau_max=4, alpha_stale=0.00, delay_sim_max_s=0.0,
    epsilon_trigger=1e-6, beta_et=0.0,
    trim_ratio=0.00, wmin= None, wmax= None,
    zeta0=None, gamma0=0.5,  # if zeta0 is None, set from eta*I*gamma0
    L_smooth_est=1.0,        # conservative estimate for admissible cap
    trust_region_frac= None, clip_updates=True, clip_multiplier=2.5,
    byz_frac=0.0, byz_mode="signflip", byz_scale=5.0,
    results_dir="results_rafl", eval_every=5, seed=42,
):
    ...
    test_loader   = DataLoader(test_dataset, batch_size=256, shuffle=False, num_workers=1, pin_memory=pin)
    ...
    server_losses_proxy = []   # mean local loss over transmitting clients S(t)
    ...
    for t in tqdm(range(rounds), desc="RAFL Rounds"):
        ...
        # server-loss proxy: mean of the losses among transmitters (if none, fallback to all)
        if len(S_indices) > 0:
            server_losses_proxy.append(float(np.mean([local_losses[i] for i in S_indices])))
        else:
            server_losses_proxy.append(float(np.mean(local_losses)) if local_losses else 0.0)

        # compute ζ_t and γ_t for this round
        # if there's no aggregation this round, we still "compute" γ_t for logging consistency
        ...
        # rotate history buffer and evaluate periodically
        ...
        if (t + 1) % eval_every == 0:
            ...
            test_losses.append(float(loss_te)); test_accs.append(float(acc_te))

    # --------------- Save logs on CPU ---------------
    np.save(os.path.join(results_dir, "server_losses_proxy.npy"), np.array(server_losses_proxy, dtype=np.float64))
    ...
    return dict(server_model=server_model, server_state=server_state,
                server_losses_proxy=server_losses_proxy, test_losses=test_losses,
                test_accs=test_accs, suppression_rates=suppression_rates,
                bar_taus=bar_taus, Xi_proxy=Xi_list)
...
if __name__ == "__main__":
    cfg = dict(
        num_clients=10, alpha_dirichlet=0.5, batch_size=64,
        rounds=200, clients_per_round=6, I_local=10, eta_client=6e-3,
