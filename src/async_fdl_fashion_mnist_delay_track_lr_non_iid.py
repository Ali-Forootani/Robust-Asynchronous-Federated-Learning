...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 15 14:04:06 2025
...
# -*- coding: utf-8 -*-
"""
Robust Asynchronous Federated Learning (RAFL) on Fashion-MNIST
with round-wise test evaluation and heterogeneity proxy \hat{\nu}_t^2.

This is a faithful port of your CIFAR-10 RAFL script to Fashion-MNIST.
Key changes vs CIFAR version:
- Dataset: torchvision.datasets.FashionMNIST (1x28x28) + Fashion-MNIST normalization.
- Model: small ResNet-style backbone with 1 input channel.
- Results directory: results_rafl_fmnist/...
...
# ------------------------- Model: Small ResNet for Fashion-MNIST -------------------------
...
# ------------------------- Utilities -------------------------
def create_directory(num_clients, num_rounds, local_epochs, max_clients_per_round, base_dir="results_rafl_fmnist"):
    dn = os.path.join(
        base_dir,
        f"fmnist_clients_{num_clients}_rounds_{num_rounds}_epochs_{local_epochs}_"
...
async def federated_learning_rafl(
    ...
    num_rounds=100, local_epochs=3, max_clients_per_round=5,
    ...
    for t in tqdm(range(num_rounds), desc="RAFL Rounds (Fashion-MNIST)"):
...
async def main():
    ...
    # ---- AFL mode (collapse RAFL -> standard AFL) ----
    AFL_MODE = True
    if AFL_MODE:
        enable_byzantine = False   # no Byzantine simulation
        byz_frac = 0.0             # irrelevant now
        asb_trim = False           # no trimming
        trigger_eps = 0.0          # include all updates
        alpha_stale = 0.0          # disable staleness weighting
        beta_et = 0.0              # disable event-trigger scaling
    ...
    print(f"[DONE] Fashion-MNIST results saved in: {results_dir}")
