...
Implements Algorithm up to p.31 (staleness weighting, ET, robust trim):
- Client sampling without replacement
- Local SGD/Adam for I steps from stale snapshot θ^{(t-τ_c)}
- Staleness-aware weights: raw w_c = 1/(1+α τ_c) then projection to capped simplex
  so that wmin/J <= \tilde w_c <= wmax/J and sum=1 over ALL selected clients J(t)
- τ̄_t computed with those \tilde w_c over ALL selected clients
- Event trigger S(t) = {c : ||u_c|| >= ε}, suppression p_t = 1 - |S|/J
- Robust aggregation: trim-by-distance-to-median on S(t), then weighted mean using
  the SAME staleness weights restricted to S(t) and re-normalized on the kept set
- Server step: γ_t = ζ_t / (η I), with ζ_t = ζ0 / ( sqrt(t+1) (1 + α τ̄_t) (1 + β p_t) )
  and optional cap γ_t <= 1/(4 L η I)
- Logs true server losses each round (train & test), plus diagnostics.

Author: Ali Forootani (adapted & cleaned)
"""
...
def _project_to_capped_simplex(v: torch.Tensor, lower: float, upper: float) -> torch.Tensor:
    ...
    Bisection on λ s.t. w = clip(v - λ, lower, upper) sums to 1.
    Requires n*lower <= 1 <= n*upper and lower <= upper.
    """
    n = v.numel()
    assert lower <= upper, "lower must be <= upper"
    # Bisection interval for lambda
    lo = (v - upper).min().item()
    hi = (v - lower).max().item()
    for _ in range(60):
        mid = 0.5 * (lo + hi)
    ...
    # Normalize to correct tiny numerical drift
    s = w.sum()
    w = w / (s + 1e-12)
    return w
...
def compute_capped_normalized_stale_weights(taus, alpha_stale, J, wmin_bound=0.5, wmax_bound=2.0, device="cpu"):
    """
    taus: list[int] or 1D tensor of length J (staleness for selected clients)
    ...
    v = raw / (raw.sum() + 1e-12)           # start from normalized raw
...
async def run_rafl(
    ...
    # Event trigger
    epsilon_trigger=0.0,         # start neutral; raise later (e.g., 1e-5)
    beta_et=0.0,                 # start neutral; 0.2–0.5 later

    # Robust agg
    trim_ratio=0.0,              # start neutral; 0.1–0.2 later

    # Stepsize schedule (server)
    zeta0=5e-3,                  # γ0 ≈ 0.05 when η=1e-3, I=5
    wmin_bound=0.5,
    wmax_bound=2.0,

    # Smoothness cap for γ_t <= 1 / (4 L η I)
    L_smooth_cap=250.0,
    use_gamma_cap=True,
    ...
    for t in tqdm(range(rounds), desc="RAFL Rounds"):
        ...
        assert abs(float(w_tilde_all.sum().item()) - 1.0) < 1e-6
...
if __name__ == "__main__":
    cfg = dict(
        ...
        I_local=5,
        eta_client=1e-3,
        client_optimizer="adam",     # switch to "sgd" for theory-friendly runs
        momentum=0.9,
        nesterov=True,
        ...
        # Start neutral; re-enable after sanity run
        epsilon_trigger=0.0,         # -> e.g., 1e-5 later
        beta_et=0.0,                 # -> 0.2–0.5 later
        trim_ratio=0.0,              # -> 0.1–0.2 later

        # Stepsizes (γ0 ≈ 0.05 with η=1e-3, I=5)
        zeta0=5e-3,
        wmin_bound=0.5,
        wmax_bound=2.0,
